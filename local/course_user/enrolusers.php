...
$courselabel = "Select Course(s) to enroll";
$coursecATlabel = "Select Course Category";

$where = " AND u.id IN (" . $elementList . ")";
$query = "SELECT DISTINCT(u.id), CONCAT(u.firstname,' ', u.lastname,' (',u.username,')') AS name FROM mdl_user AS u  WHERE u.suspended = 0 AND u.deleted = 0 $where";
    ...
    <div class="enrol-main">
        ...
        <div class="form-group">
            ...
            ?>
                ...
                <select class="form-control elements" id="coursecat_element_list" name="coursecat_element_list">
                    <?php
    ...
    function saveCourseElementData(assignElement, assignTo, assignId, elementList, end_date) {
        ...
        if (assignElement != '' && assignTo != '' && assignId != '' && assignId != 0 && elementList != '' && elementList != 0) {
            $.ajax({
                url: base_url + '/local/course_user/classes/ajax.php',
                type: 'POST',
                data: 'action=saveCourseEnrolmentData&assignElement=' + assignElement + '&assignTo=' + assignTo + '&assignId=' + assignId + '&elementList=' + elementList + '&end_date=' + end_date,
                success: function(data1) {
